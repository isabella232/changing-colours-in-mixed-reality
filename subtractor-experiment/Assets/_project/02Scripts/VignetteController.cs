    ...
    public class VignetteController : MonoBehaviour
    {
        ...
        void Update()
        {
            for (int i = 0; i < _vignette.Count; i++) {
                _vignette[i].VignettePower = (1 - _controllerConnectionHandler.ConnectedController.TriggerValue) * VIGNETTE_SCALE;
            }
            UpdateStatusText();
        }
        #endregion
        ...
        {
            _statusLabel.text = string.Format("Vignette Mode: {0}\n\nVignette Power: {1}", _vignette[0].VignetteState, _vignette[0].VignettePower);
        }
        #endregion
